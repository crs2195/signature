...
namespace RemoteSignatureApp.Classes
{
    public class CSC_API_Client
    {
        ...
        public async Task<string > Credentials_Info_Cert(string access_token, string credentialId)
        {
            var uri = BASE_URL + "credentials/info";
            ...
            using (var client = new HttpClient())
            {
                ...
                using (var response = await client.PostAsync(uri, content))
                {
                    ...
                    try
                    {
                        ...
                        return objResponse.cert.certificates[0];
                    }
                    catch (Exception ex)
                    {
                        return null;
        ...
        public void Credentials_SendOTP(string access_token, string credentialId)
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                using (var response = client.PostAsync(uri, content))
                {
                    ...
                    string token = responseJson.Result.Replace("\"", "");
                }
            }
        }
